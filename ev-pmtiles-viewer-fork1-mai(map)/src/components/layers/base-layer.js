...
export class BaseLayer {
  ...
  toggleLayerState(map, sidebar = null) {
    ...
    // Update layer controls button state if provided
    if (sidebar && sidebar.updateLayerToggleButton) {
      // Legacy: sidebar has updateLayerToggleButton method directly
      sidebar.updateLayerToggleButton(this.id, this.isVisible);
    } else if (sidebar && sidebar.getLayerControls) {
      // New: get LayerControls through sidebar
      const layerControls = sidebar.getLayerControls();
      if (layerControls) {
        layerControls.updateLayerToggleButton(this.id, this.isVisible);
      }
    } else if (
      sidebar &&
      typeof sidebar.updateLayerToggleButton === 'function'
    ) {
      // Direct LayerControls instance passed as sidebar
      sidebar.updateLayerToggleButton(this.id, this.isVisible);
    }
