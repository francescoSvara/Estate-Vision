...
export async function fetchParcelZonesByMunicipality(procom) {
  try {
    // _onsole.log(`🏠 Fetching parcel zones for municipality: ${procom}`);
    ...
    // _onsole.log('📦 Parcel Zones API response:', response);
...
export async function checkParcelZonesApiHealth() {
  try {
    ...
    // _onsole.log('📊 Parcel Zones API health response:', response);
    return {
      success: true,
...
export function formatParcelZonesForDisplay(parcelZones) {
  ...
  return parcelZones.map((parcelZone, index) => {
