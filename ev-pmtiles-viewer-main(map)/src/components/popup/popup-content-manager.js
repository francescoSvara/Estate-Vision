...
export class PopupContentManager {
  ...
  getPopupContent(coordinates, clickEvent) {
    ...
    if (intersectedLayers.length > 0) {
      ...
      return {
        type: 'layer',
        html: this.generateLayerContent(intersectedLayers),
  ...
  generateLayerContent(intersectedLayers) {
    ...
    if (intersectedLayers.length === 1) {
      ...
      // _onsole.log(intersectedLayers[0].properties);
      const p = intersectedLayers[0].properties;
      // If `layerInfo.layerId` start with `parcels-m%` (handled above)

      if (layerInfo.layerId.startsWith('parcels-m')) {
        // Example properties for parcels-m%:
        // bt_pg_r00_21_sez21_id:580911010272
        // inspireid_localid_2:"H501"
        // inspireid_localid_3:"050000"
        // municipality_zona_omi:"H501|B4|019360"
        // particella:"113"
        // pid_pg_parcels_251001:70258648
        // pop21_sqkm:823.544457676918
        content += `
          <div class="popup-layer-single">
            <strong>Parcel Information</strong>
            <div class="layer-details">
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">ISTAT SEZ.ID</span>
                <span class="layer-value">${p.bt_pg_r00_21_sez21_id}</span>
              </div>
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">BELFIORE</span>
                <span class="layer-value">${p.inspireid_localid_2}</span>
              </div>
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">CAT.FOGLIO</span>
                <span class="layer-value">${p.inspireid_localid_3}</span>
              </div>
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">CAT.PARTICELLA</span>
                <span class="layer-value">${p.particella}</span>
              </div>
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">ZONA O.M.I.</span>
                <span class="layer-value">${p.municipality_zona_omi}</span>
              </div>
              <div class="layer-item">
                <span class="layer-label" style="margin-right: 0.5rem;">POP/SQKM (2021)</span>
                <span class="layer-value">${parseFloat(p.pop21_sqkm).toFixed(1)}</span>
              </div>
              <div class="layer-item" id="census-fam21-${p.bt_pg_r00_21_sez21_id}">
                <span class="layer-label" style="margin-right: 0.5rem;">FAM21</span>
                <span class="layer-value">Loading...</span>
              </div>
              <div class="layer-item" id="census-edi21-${p.bt_pg_r00_21_sez21_id}">
                <span class="layer-label" style="margin-right: 0.5rem;">EDI21</span>
                <span class="layer-value">Loading...</span>
              </div>
              <div id="omi-data-${p.bt_pg_r00_21_sez21_id}" style="margin-top: 1rem;">
                <!-- OMI Real Estate Data will be loaded here -->
              </div>
            </div>
            <button class="popup-details-button" data-parcel-id="${p.pid_pg_parcels_251001}" data-sez-id="${p.bt_pg_r00_21_sez21_id}" style="width: 100%; margin-top: 1rem; padding: 0.5rem; background-color: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">
              DETAILS
            </button>
          </div>
        `;
