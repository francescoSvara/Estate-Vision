...
export class ParcelBlocksSearch {
  ...
  zoomToParcelBlock(parcelBlock) {
    ...
    const fullFeature = this.parcelBlocks.find(
      feature =>
        ...
        feature.properties.pid_parcels_251001 === parcelBlock.pid
    );
  ...
  highlightParcel(fullFeature) {
    ...
    const pid = fullFeature.properties.pid_parcels_251001;
    ...
    if (this.parcelsLayers && this.map) {
      this.parcelsLayers.highlightParcelByPid(this.map, pid);
  ...
  clearParcelHighlight() {
    if (this.parcelsLayers && this.map) {
      this.parcelsLayers.clearParcelHighlight(this.map);
      this.highlightedParcelPid = null;
    }
  }
